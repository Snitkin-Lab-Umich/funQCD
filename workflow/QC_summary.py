...
def final_qc_summary(multiqc_path,auriclass_path,raw_coverage_path,samples_csv,predict_pass_samples):
    ...
    # subset to samples that passed funannotate prediction step
    #data_raw_coverage = data_raw_coverage[data_raw_coverage['Sample'].isin(predict_pass_samples)]
    print(data_raw_coverage)
    ...
    # subset to samples that passed funannotate prediction step
    #data_trim_fastqc = data_trim_fastqc[data_trim_fastqc['Sample'].isin(predict_pass_samples)]
    print(data_trim_fastqc)
    print(data_trim_fastqc.columns)
    ...
    # subset to samples that passed funannotate prediction step
    #data_quast = data_quast[data_quast['Sample'].isin(predict_pass_samples)]
    print(data_quast)
    ...
    # subset to samples that passed funannotate prediction step
    #data_busco_merge = data_busco_merge[data_busco_merge['Sample'].isin(predict_pass_samples)]
    print(data_busco_merge)

    # load auriclass report
    data_auriclass = pd.read_csv(auriclass_path, sep = '\t', header = 0)
    # subset to samples that passed funannotate prediction step
    #data_auriclass = data_auriclass[data_auriclass['Sample'].isin(predict_pass_samples)]
    ...
    # subset to samples that passed funannotate prediction step
    #data_samples = data_samples[data_samples['Sample'].isin(predict_pass_samples)]
    print(data_samples)
    ...
    # subset to samples that passed funannotate prediction step
    data_merge = data_merge[data_merge['Sample'].isin(predict_pass_samples)]
    print(data_merge)
    ...
    for c in fastqc_tests:
        if c not in data_merge.columns:
            data_merge[c] = 'NA'
    data_merge['fastqc_tests_passed'] = [sum(data_merge.iloc[x][fastqc_tests] == 'pass') for x in range(len(data_merge.index))]
...
def main(multiqc_path, auriclass_path, raw_coverage_path, output_path, 
         ...
         fastqc_tests_passed_min, busco_n_score_min, assembly_qc_file, prediction_qc_file, samples_csv, predict_pass_samples):
    ...
    final = final_qc_summary(multiqc_path = mult, auriclass_path = auri, raw_coverage_path = rcov, samples_csv = samples_csv, predict_pass_samples = predict_pass_samples)
...
if __name__ == "__main__":
    ...
    predict_pass_samples = snakemake.params["predict_sample_vec"]
    main(
        ...
        assembly_qc_file=assembly_qc_file,prediction_qc_file=prediction_qc_file, samples_csv=samples_csv, predict_pass_samples=predict_pass_samples
        )
